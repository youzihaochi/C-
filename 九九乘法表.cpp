...
int main()//返回值为整数
{
	int i, j, result;//定义i.j.result为整数
	for(i=1;i<10;i++)// i=1是初始化部分；i<10是循环判断条件部分（当满足此条件时才进入执行for循环中的语句）；i++是执行完循环体语句后的操作
	{
		for(j = 1; j <= i;j++)// j=1是初始化部分；j<i是循环判断条件部分（当满足此条件时才进入执行for循环中的语句）；j++是执行完循环体语句后的操作
		{
			result = i * j;
			printf("%d*%d=%-3d", i, j, result);//提示语句，输出九九乘法表
		}
		printf("\n");//提示语句，在输出时有空格
	}
	return 0;//安全返回
